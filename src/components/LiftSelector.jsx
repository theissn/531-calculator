  ...
  return (
    <div class="flex border-b border-border bg-bg overflow-x-auto no-scrollbar">
      <For each={LIFT_OPTIONS}>
        {(option) => (
          <button
            class={`flex-1 px-3 py-2 text-sm font-bold uppercase tracking-wider font-mono border-r border-border last:border-r-0 transition-colors whitespace-nowrap rounded-none ${currentLift() === option.id
              ? 'bg-text text-bg'
